...
class TYTFW(object):
    ...
    def xor(a, b):
        ...
        l = max(len(a), len(b))
        for i in range(l):
            out.append(a[i % len(a)] ^ ord(b[i % len(b)]))
        return bytes(out)

class MD2017FW(TYTFW):
    #Many thanks to KG5RKI
    #https://github.com/travisgoodspeed/md380tools/issues/789
    key = binascii.unhexlify('00aa89891f4beccf424514540065eb66417d4c88495a210df2f5c8e638edbcb9'
    ...
    def __init__(self, base_address=0x800c000):
        ...
        self.bar = (b'\x02\x19\x0C\x03\x04\x05\x06\x07'
                    b'\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f'
                    b'\x10\x11\x12\x13\x14\x15\x16\x17'
                    b'\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f'
                    b'\x20')
        self.foob = (b'\x02\x00\x00\x00\x00\x00\x06\x00')
    ...
    if args.wrap:
        ...
        try:
            ...
            for i in range(0, 256, 16):
                ...
                hl = b' '.join(hl[i:i + 2] for i in range(0, 32, 2))
                sys.stderr.buffer.write(hl + b'\n')
